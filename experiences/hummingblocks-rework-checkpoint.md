...
> 최종 갱신: 2026-09-19  
> 상태: 초기 Android 작업 HB-E01~E05 및 HB-01~HB-23 확인 완료  
> 다음 확인 대상: HB-24 악기 교체 기능과 저장·상태 버그 수정
...
다음 작업은 **HB-24 — 악기 교체(replace) 기능과 저장·상태 처리 보완**이다.

`4d37e3b`, `c22b525`, `063713c`, `1969c30`에 더해 `8ff5698`의 replace progress overlay 부분을 함께 확인한다.

- 기존 악기를 다른 악기 음원으로 교체하는 데이터 구조와 적용 시점
- 동일 대상 악기를 여러 교체 버튼이 가리킬 때 충돌을 어떻게 막았는지
- 교체 입력이 즉시가 아니라 다음 마디에 적용되는 구조와 progress overlay의 의미
- JSON/MP3 저장 시 교체 상태가 어떻게 보존됐는지
- 캐릭터 표시와 star 상태 관련 후속 버그를 어떻게 수정했는지

분류 정정:
- `3ef066b`는 HB-23의 실제 countdown 구현
- `8ff5698`은 mixed-scope 커밋: `initial_input_length 1000→2000ms`는 HB-23과 연결되고, replace 원형 progress overlay는 HB-24로 이동
- `2c0cf19`는 HB-30 대규모 접근성 UX 개편으로 유지
