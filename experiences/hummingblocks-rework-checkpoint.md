...
> 최종 갱신: 2026-09-19  
> 상태: 초기 Android 작업 HB-E01~E05 및 HB-01~HB-27 확인 완료  
> 다음 확인 대상: HB-28 첫 사용자 튜토리얼과 시작 안내 팝업
...
다음 작업은 **HB-28 — 첫 사용자 튜토리얼과 시작 안내 팝업**이다.

`9d9d330`, `35d4c6c`, `3f47bb7`, `a086476`을 중심으로 다음을 확인한다.

- 최초 실행 시 튜토리얼 진입 조건과 `Progress.tutorial_done` 연결
- 튜토리얼 화면/overlay가 어떤 단계로 구성됐는지
- 촬영·재생 화면의 실제 UI 위에 설명을 겹쳐 보여주는 구조인지
- 사용자가 튜토리얼을 건너뛰거나 다시 볼 수 있었는지
- 9월 초기 구현과 12월 후속 개편에서 무엇이 바뀌었는지

확정된 HB-27 역할 경계:
- 원본 UI 디자인/시안: 디자이너가 Figma에 제작
- 재훈님 기여: Figma의 위치·크기·비율을 계산해 Android ConstraintLayout/percentage Guideline 기반으로 구현
- 따라서 `UI 디자인 주도`가 아니라 `디자인 시안의 Android 반응형 레이아웃 구현`으로 표현
- 태블릿/Z Flip 화면비 대응은 별도 후속 작업으로 계속 추적
