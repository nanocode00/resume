...
> 최종 갱신: 2026-09-19  
> 상태: 초기 Android 작업 HB-E01~E05 및 HB-01~HB-28 확인 완료  
> 다음 확인 대상: HB-29 퀘스트 완료 조건·별 표시·해금 애니메이션
...
- 기능을 추가하게 된 직접적인 제품 요구나 사용자 피드백의 출처는 코드만으로 확인되지 않으므로 별도 확인 전에는 추정하지 않는다.
...
다음 작업은 **HB-29 — 퀘스트 완료 조건·별 표시·순차 해금·해금 애니메이션**이다.

`0881ce8`, `35d4c6c`, `164edf8`, `dc2ad19`, `37446ca`를 중심으로 다음을 확인한다.

- HB-25에서 저장만 하던 `quest_progress`가 실제 블록 조건 검증과 언제 연결됐는지
- `quest.json`의 `match_block` 조건을 어떤 방식으로 판정했는지
- 퀘스트 성공 시 progress 증가·별 표시·교체 버튼 해금이 어떻게 이어졌는지
- 같은 조건을 반복 만족했을 때 중복 해금을 어떻게 막았는지
- `dc2ad19`의 unlock animation이 어떤 상태 전환을 시각화했는지
- `37446ca`에서 퀘스트 완료 조건을 왜/어떻게 수정했는지

HB-28 확정:
- 실제 Select/Play/View Code/Save UI를 TutorialActivity에 inflate해 재사용하는 인터랙티브 튜토리얼
- CameraActivity의 실제 CV 결과를 단계별 기대 block 배열과 비교해 올바른 조립을 확인
- `Progress.tutorial_done`으로 최초 사용자 온보딩 완료 여부 영속화
- 12월에는 제품 기능 전반을 32개 interaction state로 세분화
- 약 1초 hold progress 방식의 Skip 도입

남은 미확인:
- HB-17~18의 `약 90% → 57%` 정확도 평가 데이터셋·샘플 수·집계 방식
- 속도 비교의 원본 측정표/로그가 남아 있는지 여부
- HB-24 악기 교체 기능을 추가하게 된 직접적인 제품 요구/기획 배경
- HB-25 진행도 저장 구조를 다시 설계하게 된 직접적인 사용자/기획 요구 배경
- 태블릿/Z Flip 화면비 대응의 정확한 후속 커밋·화면 범위
- HB-28 튜토리얼 교육 내용·문구·이미지 원안의 역할 분담
