...
> 최종 갱신: 2026-09-19  
> 상태: 초기 Android 작업 HB-E01~E05 및 HB-01~HB-24 확인 완료  
> 다음 확인 대상: HB-25 사용자 진행도 저장 구조·다이얼로그 재설계
...
다음 작업은 **HB-25 — 사용자 진행도 저장 구조와 관련 다이얼로그 재설계**다.

`e837550`, `28bee75`를 중심으로 다음을 확인한다.

- 기존 진행도/해금 상태가 어디에 어떻게 저장됐는지
- 어떤 문제 때문에 저장 구조를 다시 설계했는지
- 음악·퀘스트별 진행 상태를 어떤 데이터 모델로 분리했는지
- 저장/불러오기 실패나 초기 사용자 상태를 어떻게 처리했는지
- 다이얼로그가 진행도 변경 흐름과 어떻게 연결됐는지

분류 메모:
- `1969c30`은 mixed-scope 커밋으로, 교체 대상 캐릭터 reset 수정은 HB-24에 포함하고 Star 사용 여부 시각화는 별도 기능 맥락으로 본다.
- `8ff5698`의 replace progress overlay는 HB-24에 포함하고, 초기 재생 지연 1000→2000ms 변경만 HB-23과 연결한다.
