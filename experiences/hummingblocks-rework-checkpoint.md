...
> 최종 갱신: 2026-09-19  
> 상태: 초기 Android 작업 HB-E01~E05 및 HB-01~HB-14 확인 완료, HB-15~HB-16 코드 구조 확인  
> 다음 확인 대상: HB-17~18 TFLite 전환 실험
...
다음 작업은 **HB-17~18 — Java/TensorFlow Lite 전환 실험과 미채택 결정**이다.

`7cdc9e1`, `fc771fd`, `1bb3f7c`를 중심으로 다음을 확인한다.

- Chaquopy/Python 추론 경로에서 병목이 정확히 어디였는지
- TFLite 모델을 Android Java에서 직접 실행하도록 어떤 전처리·후처리를 옮겼는지
- 9.1초 → 5.4초 속도 개선과 약 90% → 57% 정확도 저하 기록의 코드·문서 근거
- 이미지 비율 유지와 padding 보정이 정확도에 어떤 영향을 줬는지
- 실험 결과 제품 경로를 Python/Chaquopy로 되돌린 판단 근거

HB-15와 HB-16의 직접적인 기획 계기는 코드 근거가 부족하므로 추후 사용자 기억 또는 당시 문서가 확보되면 보강한다.
